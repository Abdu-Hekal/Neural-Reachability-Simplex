...
# MPC parameters
R = np.diag([0.0001, 0.01])  # input cost matrix
Rd = np.diag([0.0001, 0.01])  # input difference cost matrix
Q = np.diag([0.01, 0.01, 0.05, 0.5])  # state cost matrix, for the the next (T) prediction time steps [x, y, v, yaw]
...
if __name__ == '__main__':
    ...
    while not done:
        ...
        if control_count == 10:
            ...
            #vertices_list, intersect = reach.reachability(controller.oa, controller.odelta, state, window.batch)
        ...
        if vertices_list and control_count == 1:
            for vertex_list in vertices_list:
                vertex_list.delete()
